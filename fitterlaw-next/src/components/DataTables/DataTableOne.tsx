<html><head></head><body>import React, { useMemo } from "react";
import {
  useTable,
  useSortBy,
  useGlobalFilter,
...
const DataTableOne = () =&gt; {
  ...
  return (<section classname="data-table-common rounded-sm border border-stroke bg-white py-4 shadow-default dark:border-strokedark dark:bg-boxdark"><div classname="flex justify-between px-8 pb-4"><div classname="w-100"><input type="text" value="{globalFilter}" onchange="{(e)" =="">setGlobalFilter(e.target.value)}
            ...
            placeholder="Search..."
          /&gt;</div><div classname="flex items-center font-medium"><select value="{pageSize}" onchange="{(e)" =="">setPageSize(Number(e.target.value))}
            className="bg-transparent pl-2"
          &gt;
            {[5, 10, 20, 50].map((page) =&gt; (<option key="{page}" value="{page}">{page}</option>))}</select><p classname="pl-2 text-black dark:text-white">Entries Per Page</p></div></div>{headerGroups.map((headerGroup, key) =&gt; (
          ...
          })}<table {...gettableprops()}="" classname="datatable-table datatable-one w-full table-auto !border-collapse overflow-hidden break-words px-4 md:table-fixed md:overflow-auto md:px-8"><thead classname="border-separate px-4"><tr classname="border-t border-stroke dark:border-strokedark" {...headergroup.getheadergroupprops()}="" key="{key}"><th {...column.getheaderprops(column.getsortbytoggleprops())}="" key="{key}"><div classname="flex items-center"><span>{column.render("Header")}</span><div classname="ml-2 inline-flex flex-col space-y-[2px]"><span classname="inline-block"><svg classname="fill-current" width="10" height="5" viewBox="0 0 10 5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 0L0 5H10L5 0Z" fill=""></path></svg></span><span classname="inline-block"><svg classname="fill-current" width="10" height="5" viewBox="0 0 10 5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 5L10 0L-4.37114e-07 8.74228e-07L5 5Z" fill=""></path></svg></span></div></div>{column.canFilter ? column.render("Filter") : null}</th></tr></thead><tbody {...gettablebodyprops()}=""><tr classname="border-t border-stroke dark:border-strokedark" {...row.getrowprops()}="" key="{key}"><td {...cell.getcellprops()}="" key="{key}">{cell.render("Cell")}</td></tr></tbody></table><div classname="flex justify-between border-t border-stroke px-6 pt-5 dark:border-strokedark"><div classname="flex"><button classname="flex cursor-pointer items-center justify-center rounded-md p-1 px-2 hover:bg-primary hover:text-white" onclick="{()" =="">previousPage()}
            disabled={!canPreviousPage}
          &gt;<svg classname="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12.1777 16.1156C12.009 16.1156 11.8402 16.0593 11.7277 15.9187L5.37148 9.44995C5.11836 9.19683 5.11836 8.80308 5.37148 8.54995L11.7277 2.0812C11.9809 1.82808 12.3746 1.82808 12.6277 2.0812C12.8809 2.33433 12.8809 2.72808 12.6277 2.9812L6.72148 8.99995L12.6559 15.0187C12.909 15.2718 12.909 15.6656 12.6559 15.9187C12.4871 16.0312 12.3465 16.1156 12.1777 16.1156Z" fill=""></path></svg></button>{pageOptions.map((_page, index) =&gt; (<button key="{index}" onclick="{()" =="">{
                gotoPage(index);
              }}
              className={`${
            ...
            &gt;
              {index + 1}</button>))}<button classname="flex cursor-pointer items-center justify-center rounded-md p-1 px-2 hover:bg-primary hover:text-white" onclick="{()" =="">nextPage()}
            disabled={!canNextPage}
          &gt;<svg classname="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.82148 16.1156C5.65273 16.1156 5.51211 16.0593 5.37148 15.9468C5.11836 15.6937 5.11836 15.3 5.37148 15.0468L11.2777 8.99995L5.37148 2.9812C5.11836 2.72808 5.11836 2.33433 5.37148 2.0812C5.62461 1.82808 6.01836 1.82808 6.27148 2.0812L12.6277 8.54995C12.8809 8.80308 12.8809 9.19683 12.6277 9.44995L6.27148 15.9187C6.15898 16.0312 5.99023 16.1156 5.82148 16.1156Z" fill=""></path></svg></button></div><p classname="font-medium">Showing {pageIndex + 1} 0f {pageOptions.length} pages</p></div></section>);
};

export default DataTableOne;</filterprops<employee></employee></body></html>